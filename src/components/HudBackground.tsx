...
export default function HudBackground({ isDarkMode, params }: HudBackgroundProps) {
  ...
  // Start/stop mic based on params
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // ── Audio level bar (when mic active) ──
      if (p.micReactive && audioLevel > 0) {
        const barW = 60;
        const barH = 3;
        const bx = width - 30 - barW;
        const by = 30;
